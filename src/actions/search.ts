...
import { summarizeTranscriptWithGpt, summarizeTranscriptWithGroq } from "./summarize"; // Import summarization functions
...
const prompt = ChatPromptTemplate.fromMessages([
    [
        "system",
        `You're a seasoned web researcher, adept at ferreting out information from the vast expanse of the internet. Your expertise is being enlisted to assess the veracity of a summary extracted from a YouTube video. The user will furnish you with a concise overview gleaned from the video in question. Your task is to formulate an effective search query to ascertain the accuracy of the summary. Your evaluation should focus on determining whether the summary is founded on genuine internet facts or mere speculation.`,
    ],
    ["human", "{input}"],
...
// New function to search for articles and summarize
export const searchAndSummarize = async (query: string, model: "gpt-3.5-turbo" | "gpt-4o") => {
    try {
        const searchResult = await searchUsingTavilly(query);
        const articles = JSON.parse(searchResult).map((doc: any) => doc.pageContent).join("\n-----\n");
        // Summarize the articles
        const summary = await summarizeTranscriptWithGpt(articles, model, "Article Summary", "Summary of articles retrieved from TavilySearchAPI", "");
        return summary;
    } catch (error) {
        console.error("Error in searchAndSummarize:", error);
        throw new Error("Failed to search and summarize articles.");
    }
};